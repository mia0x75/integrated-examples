...
#### &emsp;Xray 的回落应用
1. [Xray(Trojan+RAW+TLS)+Nginx\Caddy](https://github.com/lxhao61/integrated-examples/tree/main/Xray(Trojan%2BRAW%2BTLS)%2BNginx%5CCaddy)（兼容原版 Trojan 的应用。标记为 F。）
2. [Xray(VLESS+Vision+TLS)+Nginx\Caddy](https://github.com/lxhao61/integrated-examples/tree/main/Xray(VLESS%2BVision%2BTLS)%2BNginx%5CCaddy)（VLESS+Vision+TLS 应用。标记为 E。）
...
3. [Xray(M+K)](https://github.com/lxhao61/integrated-examples/tree/main/Xray(M+K))（以 REALITY Vision 为核心的组合应用。）
...
3. V2Ray/Xray 常用单一核心应用简记：A=VLESS+mKCP+seed、B=VMess+HTTPUpgrade+TLS、C=VMess+WebSocket+TLS、D=Trojan+HTTP+TLS、E=VLESS+Vision+TLS、F=Trojan+RAW+TLS、G=Shadowsocks+gRPC+TLS、H=VLESS+XHTTP+TLS、K=VLESS+XHTTP+REALITY、M=VLESS+Vision+REALITY。
